...
$(document).ready(function() {
	...
	Janus.init({debug: "all", callback: function() {
		...
		janus = new Janus(
					...
					janus.attach(
						{
							...
							onremotestream: function(stream) {
								...
								if($('#remotevideo').length === 0) {
									addButtons = true;
									$('#stream').append('<video autoplay="true" muted="muted" class="rounded centered hide" id="remotevideo" width="100%" height="100%" playsinline/>');
			...
			document.getElementById('mute').onclick = function toggleMute() {
...
function startStream() {
	...
	if(!selectedStream) {
		bootbox.alert("Stream not available. Refresh.");
		return;
	}
	var body = { request: "watch", id: parseInt(selectedStream) || selectedStream};
	streaming.send({ message: body });
	// No remote video yet
	$('#stream').append('<video autoplay="true" muted="muted" class="rounded centered" id="waitingvideo" width="100%" height="100%" />');
	if(spinner == null) {
